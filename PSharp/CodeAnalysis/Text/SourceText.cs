...
namespace PSharp.CodeAnalysis.Text;
...
public sealed class SourceText
{
    ...
    public static ImmutableArray<TextLine> ParseLines(SourceText sourceText, string text)
    {
        ...
        {
            var lineBreakWidth = GetLineBreakWitdh(text, position);
    ...
    private static int GetLineBreakWitdh(string text, int position)
    {
    ...
    //public string ToString(int start, int lenght) => _text.Substring(start, lenght);

    public string ToString(int start, int length)
    {
        if (length == 0) return string.Empty;  // ← add this guard
