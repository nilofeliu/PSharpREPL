using PSharp.CodeAnalysis;
using PSharp.CodeAnalysis.Diagnostics;
using PSharp.CodeAnalysis.Syntax.Green;
using PSharp.CodeAnalysis.Syntax.Kind;
using PSharp.CodeAnalysis.Syntax.Nodes;
using PSharp.CodeAnalysis.Syntax.Green.Expressions;
using System.Collections.Immutable;

namespace PSharp.CodeAnalysis.Syntax.Nodes.Expressions
{
    public sealed class PostDecrementExpressionSyntax : ExpressionSyntax
    {
        private readonly GreenPostDecrementExpression _green;

        private ExpressionSyntax _operand;

        internal PostDecrementExpressionSyntax(GreenPostDecrementExpression green, SyntaxNode? parent, int position)
            : base(parent, green, position)
        {
            _green = green;
        }

        public override SyntaxKind Kind => SyntaxKind.PostDecrementExpression;

        public ExpressionSyntax Operand
        {
            get
            {
                if (_operand == null)
                    _operand = (ExpressionSyntax)RedNodeFactory.CreateRed(_green.Operand, this, GetChildPosition(0));
                return _operand;
            }
        }

        public SyntaxToken MinusMinusToken
            => new SyntaxToken(_green.MinusMinusToken, this, GetChildPosition(1));

    }
}
